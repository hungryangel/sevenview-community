...
the existing Community workspace screenshot and brand typography, not a newly
generated person. With the dev server on port 4390, run
`node scripts/render-share-card.mjs` to reproduce it (Chrome required).
