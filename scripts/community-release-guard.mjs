...
export async function inspectCommunityRelease(root = process.cwd()) {
  ...
  for (const absolute of await walk(canonicalRoot)) {
    ...
    if (path === ".env" || path.startsWith(".env.") || path.startsWith(".vercel/") || path.startsWith(".omo/")) findings.push({ code: "private-path", path })
    if (FORBIDDEN_PATH_PARTS.some((part) => path.toLowerCase().includes(part))) findings.push({ code: "forbidden-family", path })
    if (path.endsWith(".map")) findings.push({ code: "source-map", path })
    const stat = await lstat(absolute)
    if (stat.isSymbolicLink()) {
      const target = await realpath(absolute)
      if (target !== canonicalRoot && !target.startsWith(`${canonicalRoot}${sep}`)) findings.push({ code: "external-symlink", path })
      continue
    }
    ...
    if (binary) {
      ...
      if (!APPROVED_BINARY.has(path) && !path.startsWith("dist/wasm/") && !generatedFont) findings.push({ code: "unapproved-binary", path })
      continue
    }
    if (path === "scripts/community-release-guard.mjs" || path === "test/community-release-guard.test.ts") continue
    const content = data.toString("utf8")
    for (const marker of FORBIDDEN_CONTENT) if (content.includes(marker)) findings.push({ code: "forbidden-payload", path })
    if (/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/.test(content)) findings.push({ code: "secret-shaped-content", path })
  }
  return findings
}
